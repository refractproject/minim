  ...
  var ObjectElement = ArrayElement.extend({
    constructor: function (content, meta, attributes) {
      ...
      var convertedContent = _.keys(content).map(function(key) {
        var member = new MemberElement(key, content[key]);
        member.value.parents.push(self);
        return member;
      });
    ...
    set: function(keyOrObject, value) {
      ...
      if (member) {
        member.value = value;
        storedValue = member.value;
      } else {
        var newMember = new MemberElement(key, value);
        storedValue = newMember.value;
        this.content.push(newMember);
      }

      if (!_.contains(storedValue.parents, this)) {
        storedValue.parents.push(this);
      }
