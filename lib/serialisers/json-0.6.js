...
var EnumElement = require('../elements/enum-element');
...
module.exports = createClass({
  ...
  deserialise: function(value) {
    ...
    if (element.element === 'enum') {
      ...
      if (element.content) {
        element.enumerations = element.content;
      }
      ...
      if (samples) {
        ...
        existingSamples.forEach(function(sample) {
          sample.forEach(function (sample) {
            var enumElement = new EnumElement(sample);
      ...
      if (defaultValue && defaultValue.length > 0) {
        var defaultElement = new EnumElement(defaultValue.get(0));
