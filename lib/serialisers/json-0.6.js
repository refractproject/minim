...
class JSON06Serialiser {
  ...
  enumSerialiseAttributes(element) {
    ...
    const enumerations = attributes.remove('enumerations') || new ArrayElement([]);
    ...
    let samples = attributes.get('samples') || new ArrayElement([]);
  ...
  deserialiseContent(content) {
    if (content) {
      if (content.element) {
        ...
        const pair = new KeyValuePair(this.deserialise(content.key));
...
module.exports = JSON06Serialiser;
